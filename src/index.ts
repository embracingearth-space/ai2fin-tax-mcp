...
function freshness(): string {
  const ageDays = (Date.now() - Date.parse(`${VERIFIED_ON}T00:00:00Z`)) / 86_400_000;
...
const TOOLS = [
  {
    ...
    description: "Look up a country's headline tax rates with Fin: GST/VAT standard + reduced rates, top personal income tax rate, and company tax rate. Pass a 2-letter ISO country code (e.g. AU, GB, DE).",
    inputSchema: {
      type: 'object',
      properties: { country: { type: 'string', description: '2-letter ISO country code, e.g. AU' } },
      required: ['country'],
    },
  },
  {
    name: 'compute_gst_vat',
    description: "Add or remove a country's GST/VAT on an amount. mode 'add' treats the amount as tax-exclusive (adds tax on top); 'remove' treats it as tax-inclusive (extracts the tax).",
    inputSchema: {
      type: 'object',
      properties: {
        country: { type: 'string', description: '2-letter ISO country code, e.g. AU' },
        amount: { type: 'number', description: 'The amount in local currency' },
        mode: { type: 'string', enum: ['add', 'remove'], description: "'add' (exclusive) or 'remove' (inclusive)" },
      },
  ...
  },
  {
    ...
    description: 'Compare GST/VAT, income and company tax across several countries. Pass an array of 2-letter ISO country codes.',
    inputSchema: {
      type: 'object',
      properties: { countries: { type: 'array', items: { type: 'string' }, description: 'e.g. ["AU","GB","SG"]' } },
      required: ['countries'],
    },
  },
  {
    name: 'income_tax_estimate',
    description: 'Estimate income tax and take-home pay for Australia or New Zealand. Pass a 2-letter country code (AU or NZ) and gross annual income; optional taxYear like "2025-26".',
    inputSchema: {
      type: 'object',
      properties: {
        country: { type: 'string', description: 'AU or NZ' },
        income: { type: 'number', description: 'Gross annual income in local currency' },
        taxYear: { type: 'string', description: 'Optional, e.g. "2025-26"; defaults to the current financial year' },
      },
  ...
  },
  {
    ...
    description: "Estimate company/corporate tax on a profit for a source-verified country (AU, US, GB, IN, CA). Pass country, profit, and optional smallBusiness — the base/concessional rate is applied only where it covers all income (AU, IN).",
    inputSchema: {
      type: 'object',
      properties: {
        country: { type: 'string', description: 'AU, US, GB, IN or CA' },
        profit: { type: 'number', description: 'Taxable profit in local currency' },
        smallBusiness: { type: 'boolean', description: "Apply the all-income base/concessional rate (AU base-rate entity, IN s.115BAA)" },
      },
  ...
  {
    name: 'cgt_estimate',
    description: 'Estimate Australian capital gains tax for a resident individual. CGT is taxed at the marginal rate, so this differences two income-tax calcs. Pass other taxable income, sale proceeds, cost base, optional capitalLosses, and heldMoreThan12Months (the 50% discount).',
    inputSchema: {
      type: 'object',
      properties: {
        income: { type: 'number', description: 'Your other taxable income (salary etc.)' },
        proceeds: { type: 'number', description: 'Sale price of the asset' },
        costBase: { type: 'number', description: 'What you paid (cost base)' },
        capitalLosses: { type: 'number', description: 'Optional capital losses to offset, applied before the discount' },
        heldMoreThan12Months: { type: 'boolean', description: 'Whether the asset was held over 12 months (50% discount)' },
      },
...
function callTool(name: string, args: Record<string, unknown>): string {
  if (name === 'tax_rate_lookup') {
    ...
    if (!r) return `Fin doesn't have tax data for "${args.country}". Try a 2-letter ISO code like AU, GB or DE.`;
  ...
  if (name === 'compute_gst_vat') {
    const r = byCode(args.country);
    if (!r) return `Fin doesn't have GST/VAT data for "${args.country}".`;
    if (r.family !== 'GST' && r.family !== 'VAT') {
      return `${r.country} uses ${r.family} (${r.localName}), not a single-rate GST/VAT this tool can add or remove. Try tax_rate_lookup instead.`;
    }
    if (!(r.standardRate > 0)) return `${r.country} has no national GST/VAT to compute (${r.family}).`;
    const amount = Number(args.amount);
    if (!Number.isFinite(amount) || amount < 0) return 'Please pass a valid non-negative amount.';
  ...
  if (name === 'compare_countries') {
    ...
    if (!found.length) return 'Pass an array of 2-letter ISO country codes, e.g. ["AU","GB"].';
    ...
    return [`Tax comparison (headline rates, ${freshness()}):`, ...lines, DISCLAIMER].join('\n');
  }

  if (name === 'income_tax_estimate') {
    const income = Number(args.income);
    if (!Number.isFinite(income)) return 'Please pass a valid numeric income.';
    const r = estimateIncome(args.country, income, typeof args.taxYear === 'string' ? args.taxYear : undefined);
    if (!r) return `Fin has income-tax data for AU and NZ only — not "${args.country}". Use tax_rate_lookup for other countries' headline rates.`;
    ...
    lines.push(srcLine(args.country)); // source + date; the GST fact would be off-topic on a take-home figure
  ...
  if (name === 'company_tax_estimate') {
    ...
    if (!Number.isFinite(profit)) return 'Please pass a valid numeric profit.';
    if (profit < 0) return 'A loss is not taxed — company tax applies to positive profit only (losses generally carry forward, which this tool does not model).';
    const r = estimateCompany(args.country, profit, args.smallBusiness === true);
    if (!r) return `Fin has source-verified company-tax data for AU, US, GB, IN and CA only — not "${args.country}".`;
    return [
  ...
  if (name === 'cgt_estimate') {
    ...
    if (![income, proceeds, costBase, losses].every((n) => Number.isFinite(n)) || typeof args.heldMoreThan12Months !== 'boolean') {
      return 'Could not estimate CGT — pass income, proceeds, costBase and heldMoreThan12Months.';
    }
    const r = estimateAuCgt(income, proceeds, costBase, losses, args.heldMoreThan12Months);
    if (!r) return 'Could not estimate CGT — pass income, proceeds, costBase and heldMoreThan12Months.';
...
const SERVER_INFO = { name: 'ai2fin-tax', title: '2Fin Tax MCP', version: '0.1.0' };
...
function asciiCard(): string {
  ...
  ${'─'.repeat(54)}
     ...
     (    o       )             ${ROWS.length} countries · 6 tools ·
...
function handle(msg: any): object | null {
  ...
  switch (msg.method) {
    case 'initialize':
      return rpc(msg.id, { protocolVersion: PROTOCOL_VERSION, capabilities: { tools: {} }, serverInfo: SERVER_INFO, instructions: INSTRUCTIONS });
