...
function callTool(name, args) {
    if (name === 'tax_rate_lookup') {
        ...
        if (!r)
            return `Fin doesn't have tax data for "${args.country}". Try a 2-letter ISO code like AU, GB or DE.`;
    ...
    if (name === 'compute_gst_vat') {
        const r = byCode(args.country);
        if (!r)
            return `Fin doesn't have GST/VAT data for "${args.country}".`;
        ...
        if (!(r.standardRate > 0))
            return `${r.country} has no national GST/VAT to compute (${r.family}).`;
        const amount = Number(args.amount);
        if (!Number.isFinite(amount) || amount < 0)
            return 'Please pass a valid non-negative amount.';
    ...
    if (name === 'compare_countries') {
        ...
        if (!found.length)
            return 'Pass an array of 2-letter ISO country codes, e.g. ["AU","GB"].';
    ...
    if (name === 'income_tax_estimate') {
        ...
        if (!Number.isFinite(income))
            return 'Please pass a valid numeric income.';
        const r = estimateIncome(args.country, income, typeof args.taxYear === 'string' ? args.taxYear : undefined);
        if (!r)
            return `Fin has income-tax data for AU and NZ only — not "${args.country}". Use tax_rate_lookup for other countries' headline rates.`;
    ...
    if (name === 'company_tax_estimate') {
        ...
        if (!Number.isFinite(profit))
            return 'Please pass a valid numeric profit.';
        if (profit < 0)
            return 'A loss is not taxed — company tax applies to positive profit only (losses generally carry forward, which this tool does not model).';
        const r = estimateCompany(args.country, profit, args.smallBusiness === true);
        if (!r)
            return `Fin has source-verified company-tax data for AU, US, GB, IN and CA only — not "${args.country}".`;
        return [
